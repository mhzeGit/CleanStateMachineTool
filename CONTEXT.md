...
- Unity 6000.0+ required; external dependency: `com.mhze.argevent` (ArgEvent system, https://github.com/mhzeGit/ArgEventSystem); MIT licensed
...
**`StateMachineComponent.cs`** — The main runtime MonoBehaviour. Attach to any GameObject with a `StateMachineController` reference. At initialization, clones blackboard variables, syncs the `_blackboardEvents` list from the controller (preserving existing UnityEvent/ArgEvent assignments), builds the initial state path by walking from the root entry through sub-state entries to the deepest leaf, and calls `OnStateEnter` on all behaviours in the path. Each `Update()`, calls `OnStateUpdate` on all behaviours of the leaf state, then evaluates Any State (global) transitions first, followed by bottom-up per-state transitions. Each connection has a `MinStateTime` — if set, the transition is blocked until that many seconds have elapsed since entering the current leaf state. When all conditions on a connection return true (and the cooldown has passed), it transitions: exits all behaviours from the divergent path segment, fires events, enters all behaviours on the new segment, and logs a `TransitionRecord`. Public API: typed getter/setters for all six blackboard variable types, `InvokeEvent(string eventName)` to invoke a named blackboard event (dispatches UnityEvent.Invoke() or ArgEvent.Invoke() depending on EventType), `InvokeArgEvent(string eventName, object parameter)` to invoke an ArgEvent with a parameter value passed to each listener, `SyncEventNamesFromController()` to re-sync event names from the controller, `SetState(string stateName)`, `ResetStateMachine()`, properties for current state name/index/path, and a `List<TransitionRecord>` log. External reference state nodes execute the configured `ExternalStateMachineAction` on the referenced target `StateMachineComponent` when entered. Serialized `_blackboardEvents` list exposes event fields in the inspector — UnityEvent or ArgEventBinding depending on the event's `EventType`.
...
**`BlackboardEvent.cs`** — A `[Serializable]` class representing a named blackboard event that can be either a UnityEvent or an ArgEvent (from the `com.mhze.argevent` package). Holds `string Name`, `BlackboardEventType EventType` (UnityEvent/ArgEvent), `UnityEvent unityEvent`, and `ArgEventBinding argEvent`. `Clone()` creates a copy preserving the event type.
...
**`InvokeUnityEventStateBehaviour.cs`** — A built-in `StateBehaviour` subclass that invokes a named blackboard event (UnityEvent or ArgEvent) on `OnStateEnter`, `OnStateUpdate`, or `OnStateExit`. Fields: `InvokeTiming invokeOn` (OnEnter/OnUpdate/OnExit), `BlackboardEventSelector targetEvent`. Calls `stateMachine.InvokeEvent(targetEvent.EventName)` at the configured timing. `StateMachineComponent.InvokeEvent` handles both UnityEvent and ArgEvent dispatch.

**`InvokeArgEventStateBehaviour.cs`** — A built-in `StateBehaviour` subclass that invokes a named ArgEvent, optionally passing a blackboard variable value as the listener parameter. Fields: `InvokeTiming invokeOn` (OnEnter/OnUpdate/OnExit), `BlackboardEventSelector targetEvent`, `BlackboardVariableSelector parameterVariable`. When a parameter variable is set, calls `stateMachine.InvokeArgEvent(targetEvent.EventName, variableValue)`, otherwise falls back to `InvokeEvent`. The parameter value is resolved from the state machine's blackboard at invoke time and forwarded to each ArgEvent listener as the event argument.
...
**`BlackboardPanel.cs`** — The blackboard sub-panel VisualElement. Has two tabs: "Variables" and "Events". The Variables tab lists all blackboard variables with type-appropriate inline editors (toggle for Bool, numeric fields, text field, trigger toggle). The Events tab lists all blackboard events with a type badge ("UnityEvent" or "ArgEvent"). Supports: adding variables via a dropdown (all types), adding events via a type-picker menu (UnityEvent or ArgEvent), inline rename on double-click/F2, row selection (click) and deletion (Delete key), right-click context menu to convert events between UnityEvent/ArgEvent types, drag-to-reorder rows via `DragHandle`, and inline value editing for variables. Mutations use undoable commands (`DeleteBlackboardVariableCommand`, `ModifyBlackboardVariableCommand`, `DeleteBlackboardEventCommand`, `RenameBlackboardEventCommand`).
...
**`StateMachineComponentEditor.cs`** — Custom `Editor` for `StateMachineComponent` (`[CustomEditor(typeof(StateMachineComponent))]`). Shows the assigned controller with "Open Graph" button to launch the graph editor, the current state name (live-updating), an editable list of all blackboard variables with type-appropriate input fields, and a "Blackboard Events" foldout listing each named event. For UnityEvent-type events, shows the UnityEvent `PropertyField` for standard scene-object listener assignment. For ArgEvent-type events, shows the `ArgEventBinding` field (rendered by the ArgEvent package's custom property drawer) with no extra UI. Event names are auto-synced from the controller when it changes, preserving existing event field assignments. Uses `EditorApplication.update` for periodic refresh at runtime.
