...
**`BlackboardEvent.cs`** — A `[Serializable]` class representing a named ArgEvent on the blackboard. Holds `string Name` and `ArgEventBinding argEvent` (from the `com.mhze.argevent` package). `Clone()` creates a copy. Used by `StateMachineComponent` to expose assignable ArgEvent bindings in the inspector, synced from the controller's event definitions on initialization.
...
**`BlackboardPanel.cs`** — The blackboard sub-panel VisualElement. Has two tabs: "Variables" and "Events". The Variables tab lists all blackboard variables with type-appropriate inline editors (toggle for Bool, numeric fields, text field, trigger toggle). The Events tab lists all blackboard events with an "ArgEvent" type badge. Supports: adding variables via a dropdown (all types), adding events directly, inline rename on double-click/F2, row selection (click) and deletion (Delete key), drag-to-reorder rows via `DragHandle`, and inline value editing for variables. Mutations use undoable commands (`DeleteBlackboardVariableCommand`, `ModifyBlackboardVariableCommand`, `DeleteBlackboardEventCommand`, `RenameBlackboardEventCommand`).
