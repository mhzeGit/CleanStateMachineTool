...
**`BlackboardEvent.cs`** — A `[Serializable]` class representing a named UnityEvent on the blackboard. Holds `string Name` and `UnityEvent unityEvent`. `Clone()` creates a shallow copy (preserving UnityEvent reference). Used by `StateMachineComponent` to expose assignable UnityEvents in the inspector, synced from the controller's event definitions on initialization.
...
**`InvokeUnityEventStateBehaviour.cs`** — A built-in `StateBehaviour` subclass that invokes a named blackboard UnityEvent on `OnStateEnter`, `OnStateUpdate`, or `OnStateExit`. Fields: `InvokeTiming invokeOn` (OnEnter/OnUpdate/OnExit), `BlackboardEventSelector targetEvent`. Calls `stateMachine.InvokeEvent(targetEvent.EventName)` at the configured timing.
...
**`StateMachineComponentEditor.cs`** — Custom `Editor` for `StateMachineComponent` (`[CustomEditor(typeof(StateMachineComponent))]`). Shows the assigned controller with "Open Graph" button to launch the graph editor, the current state name (live-updating), an editable list of all blackboard variables with type-appropriate input fields, and a "Blackboard Events" foldout listing each named event with its UnityEvent field (using `PropertyField` for standard UnityEvent rendering, allowing scene-object listener assignment). Event names are auto-synced from the controller when it changes, preserving existing UnityEvent assignments. Uses `EditorApplication.update` for periodic refresh at runtime.
