...
**`StateMachineData.cs`** — Defines the five serializable data model classes that represent the entire state machine graph. `SerializableData` is the root container: it holds `List<StateData>` (each with name, position, size, entry/sub-entry/sub-machine flags, child indices for nesting, and a behaviour type name + instance reference), `List<ConnectionData>` (each with from/to state indices and a `List<ConditionEntry>` of conditions with type names + instance references), `List<GroupData>` (label, color, member state indices), `List<BlackboardVariable>` (typed variables), plus viewport state (pan offset, zoom, side panel width/visibility). All classes are `[Serializable]` plain data objects with no methods — they are the single source of truth that gets serialized to disk inside the controller asset.
...
**`GraphContextMenu.cs`** — Right-click context menu builder. `Show()` constructs a `MenuDropdown` with context-sensitive items: Create State, Create Sub State Machine, Connect (on state), Copy, Paste (when clipboard has data), Delete (on selection), Ungroup (on group). Also invokes registered `IContextMenuProvider` extensions. Fires action events (`CreateStateRequested`, `DeleteRequested`, etc.) that the window subscribes to.

### Graph Visual Elements

**`StateView.cs`** — VisualElement representing a single state node. Renders a rounded rectangle with shadow, selection glow (yellow highlight), fill (dark gray by default, green for entry, orange/brown for sub-machine, gold for sub-entry, lighter for active), a centered name label, and a sub-machine icon. Implements `ISelectable` for the selection system. Features: inline rename (double-click or F2) showing a text field with the old name captured for undo, rounded-rectangle hit-test that accounts for corner radius, `UpdateTransform()` for zoom/pan recalculation, `ReactivateFlash()` for active-state glow animation. Exposes `Position`, `Size`, `Name`, `IsEntry`, `IsSubEntry`, `IsSubStateMachine`, `IsSelected`, `IsActive`, `BehaviourScript`, `BehaviourInstance`, `ChildIndices`, `DataIndex`, and an `EditingCommitted` event.
...
**`DetailsPanel.cs`** — The inspector VisualElement inside the side panel. For a single state selection, displays: state name, position, behaviour script assignment (with a dropdown picker and serialized property editors for the behaviour's fields), and an "Enter Sub State Machine" button if the state is a sub-machine. For a single connection selection, displays: the transition direction and a list of condition entries (each showing script picker, serialized properties, and a remove button), plus an "Add Condition" button. For a single group selection, displays: label, color picker, member list. For multi-selection, shows a summary badge list. All property editing reads/writes via `SerializedObject` on the ScriptableObject instances.
