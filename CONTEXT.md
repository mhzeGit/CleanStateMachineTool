...
- `CleanStateMachine.Runtime` — models, runtime execution, abstract bases (no UnityEditor dependency); references `com.mhze.argevent.Runtime` for ArgEvent support
- `CleanStateMachine.Editor` — the graph editor window and all editor UI (references Runtime transitively through Runtime, Editor-only platform)
...
**`StateMachineComponent.cs`** — The main runtime MonoBehaviour. Attach to any GameObject with a `StateMachineController` reference. At initialization, clones blackboard variables, syncs the `_blackboardEvents` list from the controller (preserving existing ArgEvent assignments, also syncing argument definitions), builds the initial state path by walking from the root entry through sub-state entries to the deepest leaf, and calls `OnStateEnter` on all behaviours in the path. Each `Update()`, calls `OnStateUpdate` on all behaviours of the leaf state, then evaluates Any State (global) transitions first, followed by bottom-up per-state transitions. Each connection has a `MinStateTime` — if set, the transition is blocked until that many seconds have elapsed since entering the current leaf state. When all conditions on a connection return true (and the cooldown has passed), it transitions: exits all behaviours from the divergent path segment, fires events, enters all behaviours on the new segment, and logs a `TransitionRecord`. Public API: typed getter/setters for all six blackboard variable types, `InvokeEvent(string eventName)` to invoke a named blackboard ArgEvent (calls `argEvent.Invoke()` on the binding), `InvokeArgEvent(string eventName, object parameter)` (single-arg, delegates to multi-arg overload), `InvokeArgEvent(string eventName, object[] parameters)` (multi-arg — passes the array to each listener's `Invoke(object[])` for ArgumentSource.Event resolution), `SyncEventNamesFromController()` to re-sync event names and argument definitions from the controller, `SetState(string stateName)`, `ResetStateMachine()`, properties for current state name/index/path, and a `List<TransitionRecord>` log. External reference state nodes execute the configured `ExternalStateMachineAction` on the referenced target `StateMachineComponent` when entered. Serialized `_blackboardEvents` list exposes ArgEventBinding fields in the inspector.
...
**`BlackboardEventArg.cs`** — A `[Serializable]` class defining an argument for a blackboard event. Holds `string Name` and `BlackboardVariableType Type`. `Clone()` creates a copy. Stored as a list in `BlackboardEvent.Arguments` (up to 8 arguments) to declare what typed parameters an event expects.

**`ArgEventParameterAssignment.cs`** — A `[Serializable]` class that assigns a value to an event argument at invoke time. Holds `string ArgumentName` (matching the argument definition) and `BlackboardVariableReference Value` (direct value or blackboard variable reference). Used by `InvokeArgEventStateBehaviour` to provide parameter values for each argument when invoking a multi-arg event.

**`BlackboardEvent.cs`** — A `[Serializable]` class representing a named ArgEvent on the blackboard. Holds `string Name`, `ArgEventBinding argEvent` (from the `com.mhze.argevent` package), and `List<BlackboardEventArg> Arguments` (up to 8 typed argument definitions). `Clone()` creates a deep copy including arguments. Used by `StateMachineComponent` to expose assignable ArgEvent bindings in the inspector, synced from the controller's event definitions on initialization.

**`BlackboardEventSelector.cs`** — A `[Serializable]` utility class for selecting a blackboard event by name in state behaviour inspector fields. Has `string EventName`. Rendered as a dropdown button in `DetailsPanel.BuildBbEventSelectorField` — clicking shows all blackboard events from the controller.

**`InvokeArgEventStateBehaviour.cs`** — A built-in `StateBehaviour` subclass that invokes a named ArgEvent with support for up to 8 typed arguments. Fields: `InvokeTiming invokeOn` (OnEnter/OnUpdate/OnExit), `BlackboardEventSelector targetEvent`, `List<ArgEventParameterAssignment> argParameters` (one per argument definition on the selected event). At invoke time, resolves each parameter value from `argParameters` (either direct value or blackboard variable reference), collects them into an `object[]`, and calls `stateMachine.InvokeArgEvent(name, object[])`. When no arguments are defined, falls back to the zero-arg `InvokeEvent`.
...
**`CleanStateMachineWindow.cs`** — The main `EditorWindow` (opened via Tools > CleanStateMachine or by double-clicking a controller asset). It owns all controllers (`SelectionController`, `UndoRedoSystem`, `GraphPanController`, `DragController`, `SelectionBox`, `ConnectionController`, `GraphContextMenu`) and modules (`GraphOperations`, `GraphInputHandler`, `ExpandedViewManager`, `GraphSerializer`, `PlayModeTracker`, `GraphViewAnimator`), plus all UI layer VisualElements (`GridBackground`, `ConnectionArrowsLayer`, `GraphPreview`, `StateLayer`, `GroupContainer`, `SidePanelElement`, breadcrumb bar). The main loop runs in `OnGUI()` for IMGUI event dispatching (keyboard shortcuts, mouse events) and UI Toolkit callbacks for visual rendering. Manages viewport state (pan/zoom), unsaved-changes tracking, clipboard, and sub-state-machine expansion stack. Opens controllers via `OpenWithController(StateMachineController)`. `OnSelectionChange()` auto-opens the graph when a GameObject with a `StateMachineComponent` (and an assigned controller) is selected. When no controller is loaded, the window displays a centered placeholder label ("Open a State Machine Controller file or select a State Machine Component in the Hierarchy") and hides all graph elements — preventing accidental edits on nothing. Window layout (showSidePanel, sidePanelWidth, detailsHeightRatio) is stored in EditorPrefs (global) instead of per-asset — nested `LayoutPrefs` static class handles save/load.
...
**`GraphInputHandler.cs`** — Translates raw Unity `Event` data into high-level graph interactions. `HandleKeyboardShortcuts()` processes Ctrl+Z/Y (undo/redo), Ctrl+G (group), Ctrl+C/V/D (copy/paste/duplicate), F2 (rename), Delete/Backspace, C (connect mode), and H (auto-layout selected states). `HandleLeftClickInteraction()` dispatches mouse down/drag/up for select, drag-move, resize, and box-select based on hit-testing. `HandleConnectingInput()` manages the connection-drawing drag state (completing or cancelling). `HitTest()` returns the topmost selectable under a graph-space point; `HitTestState()` specifically for states; `GetResizeEdge()` for group resize handles. `PerformBoxSelection()` runs a marquee hit-test across states, connections, and groups and adds matches to the selection.

**`GraphLayout.cs`** — Static auto-layout utility for arranging selected state nodes in a layered top-to-bottom layout. `ComputeLayout()` builds a directed graph from connections between selected states, assigns layers via BFS (starting from entry state or nodes with no incoming edges), groups states by layer, and computes evenly-spaced positions within each layer. Handles unconnected nodes and cycles gracefully. Used by `GraphInputHandler` when the user presses H. Creates an undoable `MoveStatesCommand` through the undo/redo system.
...
**`DetailsPanel.cs`** — The inspector VisualElement inside the side panel. For a normal state selection, displays name, position, and a multi-slot behaviour list with script pickers, property editors, per-slot remove buttons, `DragHandle`-based drag-to-reorder handles, and a "+ Add Behaviour" button. For external-reference states, displays target GameObject field, action type dropdown, and conditional parameter fields. For a single connection selection, displays the transition direction and a list of condition entries (script picker, properties, remove button, `DragHandle`-based drag-to-reorder handles) plus an "Add Condition" button. For a single group selection, displays label, color picker, member list. For multi-selection, shows a summary badge list. Conditions and behaviours support drag-to-reorder via the `_conditionEntryElements`/`_behaviourEntryElements` reorderable lists with auto-scroll and `container.userData` index tracking. All property editing reads/writes via `SerializedObject` on the ScriptableObject instances. `BlackboardEventSelector` properties are rendered by `BuildBbEventSelectorField` — a dropdown button that lists all blackboard events from the controller. `BuildArgParameterFields` dynamically creates parameter value fields for `InvokeArgEventStateBehaviour` based on the selected event's argument definitions — syncing the `argParameters` list and rendering a `BlackboardVariableReference` field per argument. Validation checks both variable and event references for missing blackboard entries.
...
**`StateMachineComponentEditor.cs`** — Custom `Editor` for `StateMachineComponent` (`[CustomEditor(typeof(StateMachineComponent))]`). Shows the assigned controller with "Open Graph" button to launch the graph editor, the current state name (live-updating), an editable list of all blackboard variables with type-appropriate input fields, and a "Blackboard Events" foldout listing each named event with its `ArgEventBinding` field (rendered by the ArgEvent package's custom property drawer). The event name is corrected in the drawer's header label via a periodic schedule callback. Event names are auto-synced from the controller when it changes, preserving existing event field assignments. Uses `EditorApplication.update` for periodic refresh at runtime.
...
**`DeleteBlackboardEventCommand.cs`** — Clones the event at construction, removes it from the list by index. `Undo()` inserts the clone at the original index. Description: "Delete Event". Mirrors the variable counterpart for the events tab.

**`RenameBlackboardEventCommand.cs`** — Captures old and new `Name` for a `BlackboardEvent`. `Execute()` sets new; `Undo()` restores old. Description: "Rename Event". Used by `BlackboardPanel.CommitNameEdit()` in the Events tab.
