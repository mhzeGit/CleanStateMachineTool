...
**`StateView.cs`** — VisualElement representing a single state node. Renders a rounded rectangle with shadow, selection glow, fill (dark gray default, green entry, orange/brown sub-machine, gold sub-entry, blue-purple external-reference, lighter active), a centered name label, and sub-machine (↗) or external-reference (⬅) icons. Implements `ISelectable`. Features: inline rename, rounded-rectangle hit-test, `UpdateTransform()` for zoom/pan, `ReactivateFlash()` for active-state glow. Exposes `Position`, `Size`, `Name`, `IsEntry`, `IsSubEntry`, `IsSubStateMachine`, `IsExternalReference`, `IsSelected`, `IsActive`, `BehaviourEntries` (List<BehaviourEntryView>), `ChildIndices`, external reference fields, `DataIndex`, and `EditingCommitted` event. Also defines `BehaviourEntryView` (MonoScript Script + StateBehaviour Instance), which mirrors `ConditionEntryView` for per-behaviour script/instance tracking.
...
**`DetailsPanel.cs`** — The inspector VisualElement inside the side panel. For a normal state selection, displays name, position, and a multi-slot behaviour list with script pickers, property editors, per-slot remove buttons, and a "+ Add Behaviour" button. For external-reference states, displays target GameObject field, action type dropdown, and conditional parameter fields. For a single connection selection, displays the transition direction and a list of condition entries (script picker, properties, remove button) plus an "Add Condition" button. For a single group selection, displays label, color picker, member list. For multi-selection, shows a summary badge list. All property editing reads/writes via `SerializedObject` on the ScriptableObject instances.
